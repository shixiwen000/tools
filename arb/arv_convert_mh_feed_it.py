...
# monster app path
mh_feed_it_path = '/Users/einvcz/workspace/monster/flutter/mh_feed_kit/l10n/kit_ko.arb'
...
with open(mh_feed_it_path, 'rb') as mp_file:
    file_content = mp_file.read()
    mp_map = json.loads(file_content)

df = pd.read_excel(excel_file, sheet_name='mh-feed-kit')
key = df.iloc[0:, 0]
...
with open(mh_feed_it_path, 'w') as res_file:
