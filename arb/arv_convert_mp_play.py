...
# monster app path
mp_play_path = '/Users/einvcz/workspace/monster/flutter/mp-play/lib/global/l10n/app_ko.arb'
...
with open(mp_play_path, 'rb') as mp_file:
    file_content = mp_file.read()
    mp_map = json.loads(file_content)

df = pd.read_excel(excel_file, sheet_name='mp-play')
key = df.iloc[0:, 0]
...
with open(mp_play_path, 'w') as res_file:
